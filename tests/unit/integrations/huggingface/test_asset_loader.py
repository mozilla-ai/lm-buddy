...
def test_dataset_loading(xyz_dataset_artifact):
    ...
    dataset_path = build_wandb_asset_path(name=xyz_dataset_artifact.name, project="project")
    dataset_config = DatasetConfig(path=dataset_path, test_size=0.2, seed=0)
...
def test_model_loading(llm_model_artifact):
    ...
    model_path = build_wandb_asset_path(name=llm_model_artifact.name, project="project")
    model_config = AutoModelConfig(path=model_path, torch_dtype=torch.bfloat16)
