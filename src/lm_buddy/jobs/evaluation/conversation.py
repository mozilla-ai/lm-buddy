...
After witnessing dependency issues (wavedrom->markdown2[all]->fschat==0.2.37, with wavedrom
causing the error `No distribution metadata found for wavedrom==2.0.3.post3.` when trying
to generate lockfiles with pants) I am trying to simply import this file, downloaded from
https://raw.githubusercontent.com/lm-sys/FastChat/v0.2.35/fastchat/conversation.py, to test
the code without a dependency on fastchat.
